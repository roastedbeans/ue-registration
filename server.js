...
// Paths to packetrusher folder (in current directory)
...
function updateProgressDisplay() {
	const progress = serverState.totalUeCount > 0 ? `${serverState.currentUeIndex}/${serverState.totalUeCount}` : '0/0';

	const currentMsinDisplay = serverState.currentMsin ? serverState.currentMsin.toString().padStart(10, '0') : 'N/A';
...
function runSingleUE(ueIndex) {
	return new Promise((resolve) => {
		addLog(`Starting UE #${ueIndex} with MSIN: ${serverState.currentMsin.toString().padStart(10, '0')}`);

		const broadcast = (message, level = 'info') => {
			broadcastLogUpdate(message, 'packetrusher', level);
		};

		broadcast(`🚀 UE #${ueIndex} starting with MSIN ${serverState.currentMsin.toString().padStart(10, '0')}...`);
		...
		const killTimer = setTimeout(() => {
			if (!processKilled) {
				...
				console.log(`UE #${ueIndex} PacketRusher process terminated after 3 seconds`);
			}
		}, 3000);
...
async function runSequentialUEs() {
	...
	if (serverState.sessionAborted) {
		addLog(`🛑 Session aborted. Completed ${serverState.currentUeIndex - 1}/${serverState.totalUeCount} UEs`);
	} else {
		addLog(`🎉 All UEs completed! Total: ${serverState.totalUeCount} UEs processed`);
	}
...
app.post('/api/sessions/start', async (req, res) => {
	...
	if (!msinBase || msinBase.length !== 10) {
		validationErrors.push(`MSIN Base must be 10 digits. Current: "${msinBase}"`);
	}
	if (ueCount < 1 || ueCount > 100000) {
		validationErrors.push(`UE Count must be between 1 and 100000. Current: "${ueCount}"`);
	}
